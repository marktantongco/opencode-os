...
description: >
  AI routing gateway with RTK compression, tier-based provider fallback, circuit breaker, and real-time dashboard — routes requests through free-tier providers first, paid only on fallback. Use when use when setting up ai model routing, managing provider tiers, or implementing circuit breaker patterns for api resilience
  Triggers on keywords: routing, gateway, 9router, fallback, circuit-breaker, rtk.
