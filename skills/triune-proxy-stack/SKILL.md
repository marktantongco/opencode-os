---
name: triune-proxy-stack
description: >
  Maximum redundancy proxy architecture — 9Router + OmniRoute + Orcaflow with token health dashboard, auto-failover, and distributed locking for zero-downtime AI access. Use when use when building high-availability proxy stacks, implementing triple redundancy routing, or designing token health monitoring with auto-failover
  Triggers on keywords: triune, redundancy, failover, orchestrator, health, monitoring.
allowed-tools: Read, Write, Bash, Edit, Grep, Glob
user-invocable: true
mcp-servers: [filesystem, github, docker]
---

# Triune Proxy Architecture

## context

Maximum redundancy proxy architecture — 9Router + OmniRoute + Direct fallback with health checking and automatic failover between three independent proxy layers. This skill provides infrastructure-as-code patterns and operational procedures that ensure reliability, security, and observability of critical infrastructure components.
...
**Input:** "Use when building high-availability proxy stacks, implementing triple redundancy routing, or designing token health monitoring with auto-failover"
...
### Architecture
Multi-layer triune proxy stack with automatic failover between primary and backup endpoints.
