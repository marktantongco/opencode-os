...
name: secret-vault
description: >
  Unified credential vault with age encryption — manages API keys, OAuth tokens, and proxy credentials for all AI providers with rotation reminders and breach detection. Use when use when managing api keys across providers, implementing encrypted credential storage, or setting up secret rotation and breach detection workflows
  Triggers on keywords: vault, credentials, encryption, rotation, security, age.
...
**Input:** "Use when managing API keys across providers, implementing encrypted credential storage, or setting up secret rotation and breach detection workflows"
...
### Architecture
Multi-layer secret vault with automatic failover between primary and backup endpoints.
